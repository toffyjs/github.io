...
function updateTag(type, name, id) {
  ...
  if (!tag) {
    Tag[type](
      name,
      () => {
        window.openFromView('external', [name], Klasses);
      },
      e => {
        window.openFromView('external_err', [name], Klasses);
      }
    );
  } else {
...
function run(name, propsstr, style) {
  ...
  eval('props=' + (propsstr || '{}'));
  try {
    window.app = React.createElement(Klasses[name], props);
    window.parent = React.createElement('div', { style }, window.app);
    window.parentr = ReactDOM.render(window.parent, appdiv);
  } catch (e) {
    console.error(e);
  }
}
  ...
  for (var k in obj) {
    try {
      if (k === 'render') Klasses[name].prototype.render = Function(obj[k]);
        ...
        } else if (obj[k] !== 0) Klasses[name].prototype[k] = Function.apply(Function, obj[k]);
      }
    } catch (e) {
